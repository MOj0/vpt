...
void main() {
    ...
    photon.position = texture(uPosition, coord).xyz;
    vec4 directionAndBounces = texture(uDirection, coord);
    photon.direction = directionAndBounces.xyz;
    photon.bounces = uint(directionAndBounces.w + 0.5);
    photon.transmittance = texture(uTransmittance, coord).rgb;
    vec4 radianceAndSamples = texture(uRadiance, coord);
...
void main() {
    ...
    vec2 position = texture(uPositionRay, pos).xy;
